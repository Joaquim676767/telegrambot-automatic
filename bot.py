...
logging.basicConfig(level=logging.INFO)  # Nível de log INFO para mensagens comuns
...
GRUPO_INVITE_LINK = 'SEU_LINK_DE_CONVITE_DO_GRUPO'  # Link do grupo privado
VALOR_SERVICO = "26.00"  # Valor do serviço ou acesso
...
def telegram_webhook():
    ...
    print(f"Dados recebidos: {data}")  # Também usa o print para verificar no terminal

    if 'message' in data and 'text' in data['message']:
        chat_id = data['message']['chat']['id']
        message_text = data['message']['text']
        
        # Log para registrar o comando recebido
        logger.info(f"Comando recebido: {message_text} de {chat_id}")
        print(f"Comando recebido: {message_text} de {chat_id}")
...
def enviar_mensagem_start(chat_id):
    ...
    if response.status_code == 200:
        ...
        logger.error(f"Erro ao enviar mensagem de boas-vindas: {response.text}")
...
def gerar_qr_code_pix():
    ...
    payload = {
        ...
        "chave": "SUA_CHAVE_PIX",  # Chave Pix cadastrada no Gerencianet
...
def enviar_qr_code(chat_id, qr_code_path):
    ...
    if response.status_code == 200:
        logger.info('QR code enviado com sucesso')
        print('QR code enviado com sucesso')
    else:
        logger.error(f'Erro ao enviar o QR code: {response.text}')
...
def enviar_link_grupo(usuario_id):
    ...
    response = requests.post(url, data=data)
    if response.status_code == 200:
        logger.info('Link do grupo enviado com sucesso')
        print('Link do grupo enviado com sucesso')
    else:
        logger.error(f'Erro ao enviar o link do grupo: {response.text}')
